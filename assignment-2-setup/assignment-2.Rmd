...
get_core("deu") %>% # calls the dataset
  count(sex) %>% # count creates a column called 'n'
  mutate(percent = n / sum(n) * 100) # creates percent column and calculates
...
deu_df <- get_core("deu") %>%
  dplyr::mutate(age_at_death = year(as.period(interval(
    start = birth, end = death
  ))))



ggplot(deu_df, aes(age_at_death, fill = sex))+
  geom_histogram(binwidth = 5, colour = "black", position = "dodge",alpha=0.3)+
  scale_x_continuous(limits = c(30, 100), breaks = seq(0, 100, 5))+
  scale_y_continuous(limits = c(0, 150), breaks = seq(0, 200, 50))+
  theme_bw()+
  geom_vline(aes(xintercept=mean(age_at_death)),
             color="blue", linetype="dashed", size=1) +
  ggtitle("Histogram to Diplay Distibution of Age at which German Legislators Died")
